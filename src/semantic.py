...
class SemanticAnalyzer():
    ...
    def get_type(self, tree: SyntaxTree, local_scope: dict[str, CType]={}) -> CType | None:
        type_definitions = {
            # "*_pre" : [("int", ["int*"])],
            # "&_pre" : [("int*", ["int"])],
        ...
        if(tree.type == SyntaxTree.EXPRESSION):
            if(tree.expression_type == "?"):
                arg_types = [ self.get_type(child, local_scope) for child in tree.children ]
            ...
            if(tree.expression_type == "*_pre"):
                operand = self.get_type(tree.children[0], local_scope)
            ...
            if(tree.expression_type == "[]"):
                array_type = self.get_type(tree.children[0], local_scope)
                index_type = self.get_type(tree.children[1], local_scope)
        ...
        if(tree.type == SyntaxTree.WHILE_STATEMENT):
            [ self.get_type(child, local_scope) for child in tree.children ]
        ...
        if(tree.type == SyntaxTree.FUNCTION_DEFINITION):
            ...
            self.get_type(tree.body, local_scope)
            return "void"
